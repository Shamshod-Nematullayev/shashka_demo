...
io.on('connection', socket => {
    ...
    socket.on('joinRoom',async game_id => {
        ...
        socket.on('joinGame',async ({user_id, color, username})=>{
            if(color === 'red') await Game.findOneAndUpdate({_id: game_id}, {$set: {player_1: {user_id, username}}});
            if(color ==='white') await Game.findOneAndUpdate({_id: game_id}, {$set: {player_2: {user_id, username}}})

            bot.telegram.getChat(user_id).then(user => {
                io.sockets.in('room' + game_id).emit('joinedGame', {user_id, color, username: user.first_name})
